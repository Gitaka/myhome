...
<h2>Add  Property <label style="color:#336666">Listing</label></h2>
 {{Form::open(array('url'=>'agent/addlisting','method'=>'POST','files'=>'true'))}}

    {{Form::label('for','For Sale Or For Rent')}}</br>
  ...
  @if($errors->has('for'))<p style="color:red">{{$errors->first('for')}}</p>@endif
  ...
  @if($errors->has('mobile'))<p style="color:red">{{$errors->first('mobile')}}</p>@endif
       ...
       {{Form::label('prorpertyType','PropertyType')}}</br></br>
 ...
 {{Form::label('outsideSpace','OutsideSpace')}}</br>
  ...
  @if($errors->has('photo'))<p style="color:red">{{$errors->first('photo','Maximum of 15 Images accepted Only')}}</p>@endif

       {{Form::hidden('id',$agent->id)}}
       
       {{Form::submit('Post Listing',array('class'=>'btn btn-success'))}}
       {{Form::close()}}</br>       
@stop