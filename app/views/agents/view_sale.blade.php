...
<html>
   ...
   <body>
       ...
       @foreach($buy as $buy)
       <div id="info">
        <label style="font-size:160%;">{{$buy->name}}</label>
        <label style="float:right;font-size:140%">Ksh {{$buy->price}}</label></br>
          <label style="float:right;color:#0099ff;font-weight:normal;"><i class="fa fa-print"></i>&nbsp;&nbsp;
          {{HTML::linkRoute('sale_print','Print',array($buy->id))}}</label>
        <label style="font-style:italic;font-weight:normal;font-size:120%;">{{$buy->location}}</label></br></br>
       </div>
       ...
       <li id="image_{{$i}}"><img  src="{{asset($buy->photo).'/'.$images[$i]}}" ></li>

         @endfor
         </ul>
         <!--<img src="{{asset($buy->photo)}}" title="logo" "width=100% height=98%"/>-->
        </div>
        <div id="mainImage">
             <img id="mainimg" src="{{asset($buy->photo).'/'.$images[0]}}" alt="">
        </div>
         </div>
         <div id="describe">
